...
import { getStudioAccounts, getZohoAccountFromAccounts } from '../account/index';
...
describe('lookupLead', () => {
    ...
    it('returns the lead data if it exists', async () => {
        ...
        await axios.get.mockResolvedValue({ data: [leadData] });
        getStudioAccounts.mockResolvedValue([{ Account: { platform: 'zoho', accessToken: 'access_token' } }]);
        getZohoAccountFromAccounts.mockReturnValue({ Account: { platform: 'zoho', accessToken: 'access_token' } });
        const result = await lookupLead({ from: '1234567890', studioId: 'studioId' });
